...
//运行时记得修改文件路径为自己的
cv::Mat image=cv::imread("/home/wangzichen/dog_ws/src/global_plan/jueying.pgm",0);
...
void doPose(const nav_msgs::Path::ConstPtr& pose)
{
    ...
    vector<Node*> path;
    Path.clear();
    //cout<<"pose num: "<<start_vector.size()<<"  "<<goal_vector.size()<<endl;
    ...
    //发布路径
    count_n = count_n + 1;
    cout<<"====in the pathpub!===="<<endl;
    cout<<"The program has run for "<<count_n<<" times"<<endl;
...
int main(int argc, char *argv[])
{
    ...
    //全局路径发布节点
    path_pub=nh.advertise<nav_msgs::Path>("/global_path",100000);
