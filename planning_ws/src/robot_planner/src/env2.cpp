...
double cal_dist_average(vector<Point *> path)
{
    ...
    for(int i=0;i<(int)path.size();i++)
    {
        ...
        dist = sqrt((near->x-x)*(near->x-x) + (near->y-y,2)*(near->y-y,2));
    }
...
int main(int argc,char** argv)
{
    ...
    t2 = ros::Time::now();
    cout<<"esdf time durtion: "<<(t2 - t1).toSec()/10<<endl;
    ...
    save_path(route_esdf,"esdf_env2.csv");
    cout<<"*******************"<<endl;
    ...
    save_path(route_smooth,"smooth_env2.csv");
    cout<<"*******************"<<endl;
