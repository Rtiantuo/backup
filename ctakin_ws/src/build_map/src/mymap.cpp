...
{
    cout << " int the odom callback!" << endl;
    //step1 获取位姿和旋转
...
void projectDepth()
{
    cout << " in the depth project!" << endl;
        ros::Time t1,t2;
	    t1 = ros::Time::now();
        ...
        for(int u = 0;u<640;u=u+3)
        {
            for(int v=0;v<480;v=v+3)
            {
                        ...
                        Eigen::Vector3d pos = cal_point(u,v,depth);//zaiworld下的坐标

                        //起始x,y,z  --当前相机所在的世界位置
                        double sx = md.Pose(0), sy = md.Pose(1), sz=md.Pose(2);
                        GridIndex start_index = ConvertWorld2GridIndex(sx, sy, sz);   //--相机所在栅格中的坐标
                        //终点x,y,z
                        double gx = pos(0), gy = pos(1), gz = pos(2);      //--图像中每个点的世界坐标
                        GridIndex goal_index = ConvertWorld2GridIndex(gx,gy,gz);   //--每个点云的栅格坐标
                        ...
                        {
                                pMap[occ_idx] = pMap[occ_idx] - 1*scale;
                                if (pMap[occ_idx] <= 10)
                                {
                                    pMap[occ_idx] = 10;
                                }
